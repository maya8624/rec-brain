...
from app.services.sql_search import perform_sql_search
...
async def sql_search_node(state: RealEstateAgentState) -> dict:
    """
    Handles search_listings and check_availability tool calls.
    Calls perform_sql_search and wraps result as ToolMessage.
    """
    last_ai = _last_ai_message(state)
    if not last_ai:
        return {}
    ...
    for tc in last_ai.tool_calls:
        ...
        try:
            result = await perform_sql_search(question)
            ...
            content = (
                ...
                if result
                else {"success": False, "error": "SQL agent returned no result"}
            )

        except Exception as e:
            logger.exception(
                "sql_search_node | failed | question=%s", question)
            content = {"success": False, "error": str(e)}
