...
async def cancel_inspection(confirmation_id: str, config: RunnableConfig, reason: str | None = None) -> dict:
    ...
    logger.info("cancel_inspection_start", confirmation_id=confirmation_id, reason=reason)

    try:
        result = await booking_service.cancel(confirmation_id, user_id)
        logger.info("cancel_inspection_confirmed", confirmation_id=confirmation_id)
        return result.model_dump()

    except ToolValidationError as exc:
        logger.warning("cancel_inspection_validation_error", error=str(exc))
        return CancellationResult(
            success=False, id=confirmation_id, error=str(exc)
        ).model_dump()
    except BookingServiceError as exc:
        logger.error("cancel_inspection_service_error", error=str(exc))
        return CancellationResult(
            success=False, id=confirmation_id, error=str(exc)
        ).model_dump()
    except Exception as exc:
        logger.exception("cancel_inspection_unexpected_error", error=str(exc))
        return CancellationResult(
            success=False,
            id=confirmation_id,
