"""
End-to-end LangGraph flow tests.

Runs the compiled graph with mocked services where possible.
Requires a live Groq API key (LLM calls are real) and PostgreSQL (checkpointer).
...
def make_booking_service():
    ...
    mock.get_availability.return_value = [
        {"datetime": "2027-04-12 10:00", "agent_name": "Jane Smith", "available": True},
        {"datetime": "2027-04-12 14:00", "agent_name": "Jane Smith", "available": True},
    ]
...
class TestGraphFlows:
    ...
    async def test_booking_intent_flow(self, graph):
        ...
        result = await graph.ainvoke(
            {"messages": [HumanMessage(content="I'd like to book an inspection for property prop_123")]},
    ...
    async def test_search_intent_flow(self, graph):
        ...
        sql_service = SqlViewService(llm=get_llm(), db=get_db())
